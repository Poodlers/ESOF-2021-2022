  ...
  Widget generateNotifPage(notifications,context){
    ...
    if (notifs.isEmpty) {
      ...
      for(CustomNotification notif in notifs){
        ...
        tz.TZDateTime expenseLimitDate = tz.TZDateTime.parse(tz.local, matchingExpense.limitDate);
        tz.TZDateTime expenseNotifDate = expenseLimitDate.subtract(notificationAntecedence);

        expenseNotifDate = expenseNotifDate.isAfter(tz.TZDateTime.now(tz.local))
            ? expenseNotifDate :
